...
native [**formatting options**](#interface-options) from the `deno fmt` command.
...
All of the [**formatting options**](#interface-options) from the CLI counterpart
are supported, including `--check`, `--ext`, and `--config`. If a
...
Read the [**API**](#api) and the [**Options**](#interface-options) sections for
more information on the available methods and options.
...
[**caveats** when using synchronous APIs](#synchronous-performance-caveats).
...
<details><summary><b>⚠️ Synchronous Performance Caveats</b></summary>
...
- [x] **Versatile**: Formats code as strings, binary data, or template literals.
- [x] **Pain Free**: Written in strict, well-documented TypeScript, with
      extensive unit tests and a simple-to-use API.
- [x] **Compatible**: Full support for all the native formatting options of the
      `deno fmt` command. Works seamlessly with both the Deno CLI and in cloud
      environments like Deno Deploy, thanks to a
      [WASM engine](#class-wasmcontext-powered-by-dprint) powered by
      [`dprint`][dprint] (the same tool behind `deno fmt`).
- [x] **Performant**: Efficiently built, with ongoing benchmarks and
      optimizations for speed. Provides both asynchronous and synchronous APIs for
      formatting code in a range scenarios.
...
[LRU caching](#interface-cacheconfig) enabled (100 entry capacity, 30 minute
TTL), [`dprint`][dprint] WASM context is preferred, and the default
[formatting options](#interface-options) are used by all formatting operations.
...
Asynchronously format the given
[`input`](#formatter.format-param-input "input: `input`"), returning the result
as a string. The input may be a string, an ArrayBuffer, or an ArrayBuffer View
like a TypedArray or DataView. SharedArrayBuffers are supported, but are not
recommended for use with this method, as they may be mutated by other code while
the formatter is running.

| Tag     | Name                                                | Description         |
| :------ | :-------------------------------------------------- | :------------------ |
| `param` | <a id="formatter.format-param-input"> **input**</a> | The code to format. |
...
existing instance [`options`](#interface-options "options: `options`") prior to
formatting.
...
be merged with the existing instance
[`options`](#interface-options "options: `options`") prior to formatting.
...
| `param` | **`values`** | The interpolated values to be parsed and used to construct the template string. If the first or the last value are an {@linkcode Options} object, it will be merged with the existing instance {@linkcode options} prior to formatting. |
...
| `param` | **`values`** | The options or interpolated values to be parsed and used to construct the template string. If the first or the last value are an {@linkcode Options} object, it will be merged with the existing instance {@linkcode options} prior to formatting. |
...
[`options`](#interface-options "options: `options`") prior to checking the input
code.
...
| `param` | **`values`** | The interpolated values to be parsed and used to construct the template string. If the first or the last value are an {@linkcode Options} object, it will be merged with the existing instance {@linkcode options} prior to checking. |
...
| `param` | **`values`** | The options or interpolated values to be parsed and used to construct the template string. If the first or the last value are an {@linkcode Options} object, it will be merged with the existing instance {@linkcode options} prior to checking. |
...
It will be merged with the existing instance
[`options`](#interface-options "options: `options`") prior to checking the input
code.
...
public [`Formatter` class](#class-formatter).
...
`allow-run` permission is enabled and the [`Deno.Command`][deno-command-api]
**_is_ available**, the Deno CLI context will be used by default.
...
If you're using the CLI formatting engine, you might notice a much faster start
up time, but a slightly slower formatting time. This is due to the overhead of
spawning a subprocess, and the additional cost of piping data to and from the
spawned subprocess. However, the CLI implementation is still a very fast
formatter, and is often only a few milliseconds slower than the WASM
implementation.
...
#### Examples of different `ext` settings
...
| Option    | Possible Values                                        | Default  | Description                                                                                                                      |
| :-------- | :----------------------------------------------------- | :------- | :------------------------------------------------------------------------------------------------------------------------------- |
| `cache`   | `true` `false` [`CacheConfig`](#interface-cacheconfig) | `true`   | Enable or disable LRU caching of formatted code.                                                                                 |
...
| `type`    | `"wasm"` `"cli"`                                       | `"wasm"` | The formatter implementation to use. Defaults to `"wasm"` if running in a Deno Deploy context, or `"cli"` in a Deno CLI context. |
...
Supported by [`format`](#formatter.format),
[`formatSync`](#formatter.formatsync), [`check`](#formatter.check), and
[`checkSync`](#formatter.checksync).
...
###### This is an unofficial project, not affiliated with [**Deno**][deno] or [**Deno Deploy**][deno-deploy]. [**dprint**][dprint] is published by [**David Sherret**][dsherret] under the **MIT** License (you rock, David!).
