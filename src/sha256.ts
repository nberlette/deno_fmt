...
export type Format = keyof FormatTypes & string;
...
export class Sha256 {
  ...
  /** Return hash as a Uint8Array. */
  uint8Array(): Uint8Array {
    return new Uint8Array(this.arrayBuffer());
  }

  static from(message: Message, is224 = false, sharedMemory = false): Sha256 {
    return new Sha256(is224, sharedMemory).update(message);
  }

  static #format<K extends Format = "hex">(
    hash: Sha256,
    format: K,
  ): FormatTypes[K];
  static #format(hash: Sha256, format: Format) {
    switch (format) {
  ...
  static hmac<K extends Format = "hex">(
    secretKey: Message,
    message: Message,
    format: K = "hex" as K,
    is224 = false,
